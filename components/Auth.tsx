...
const Auth: React.FC<AuthProps> = ({ onLogin, onSignup, onRecover, users }) => {
  ...
  return (
    <div className="h-[100dvh] w-full overflow-y-auto no-scrollbar flex flex-col items-center justify-center p-6 relative bg-[#030611] text-slate-100 font-sans">
      ...
      <motion.div 
        ...
        className="w-full max-w-sm p-8 sm:p-10 rounded-[3rem] bg-slate-900/75 backdrop-blur-3xl border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.8),_inset_0_1px_2px_rgba(255,255,255,0.15)] relative z-10 overflow-hidden"
      >
        ...
        <div className="flex flex-col items-center text-center mb-6 mt-2">
          ...
          <p className="text-[9px] text-sky-400 font-extrabold uppercase tracking-[0.25em] mt-3 bg-blue-950/45 px-3 py-1 rounded-full border border-blue-900/40">
            Unified Portal Gateway
          </p>
        </div>

        {/* View Switches (Login Profile vs Create Account) */}
        <div className="flex gap-1.5 bg-slate-950/80 p-1.5 rounded-2xl mb-6 border border-white/5 shadow-inner">
          {[
            ...
            <button 
              ...
              className={cn(
                "flex-1 py-3 rounded-xl text-[9px] font-extrabold transition-all duration-300 uppercase tracking-widest cursor-pointer",
              ...
              {view === 'signup' && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      className="w-full pl-11 pr-5 py-3.5 bg-slate-950/80 border border-white/10 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 text-white focus:outline-none transition-all font-bold text-xs rounded-2xl uppercase text-slate-100 shadow-inner"
                  ...
                  {formData.referralCode.trim() && (
                    <div 
                      className={cn(
                        "mt-1.5 px-3.5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-wider border flex items-center gap-2 transition-all duration-300",
                        foundUpline 
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      required
                      className="w-full pl-11 pr-5 py-3.5 bg-slate-950/80 border border-white/10 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-white focus:outline-none transition-all font-bold text-xs rounded-2xl shadow-inner"
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      required
                      className="w-full pl-11 pr-5 py-3.5 bg-slate-950/80 border border-white/10 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 text-white focus:outline-none transition-all font-bold text-xs rounded-2xl shadow-inner animate-fade-in"
              ...
              {(view === 'signup' || view.startsWith('recover')) && (
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Phone size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-blue-400/80" />
                      <input
                        type="tel"
                        required
                        className="w-full pl-9 pr-3 py-3.5 bg-slate-950/80 border border-white/10 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 text-white focus:outline-none transition-all font-bold text-xs rounded-2xl shadow-inner"
                  ...
                  {view === 'signup' && (
                    <div className="space-y-1.5 relative">
                      ...
                      <div 
                        className="w-full px-3.5 py-3.5 bg-slate-950/85 border border-white/10 text-slate-200 cursor-pointer font-bold text-xs truncate flex items-center justify-between rounded-2xl shadow-inner hover:border-white/20 transition-all duration-300"
                      ...
                      <AnimatePresence>
                        {isStateOpen && (
                          <motion.div 
                            ...
                            className="absolute bottom-full left-0 w-full mb-2 border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden bg-slate-900"
                          >
                            <input 
                              type="text"
                              className="w-full px-4 py-3 border-b border-white/10 bg-slate-950 text-white text-xs focus:outline-none font-bold"
                            ...
                            <div className="max-h-36 overflow-y-auto no-scrollbar">
                              {filteredStates.map(s => (
                                <div 
                                  key={s} 
                                  className="px-4 py-2.5 text-xs font-bold cursor-pointer transition-colors hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600 hover:text-white text-slate-200"
                                  onClick={() => {
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full pl-11 pr-11 py-3.5 bg-slate-950/80 border border-white/10 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 text-white focus:outline-none transition-all font-bold text-xs rounded-2xl shadow-inner"
                      placeholder="••••••••"
              ...
              {view === 'signup' && (
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="space-y-1.5">
                    <label className="block text-[9px] font-extrabold text-sky-400 uppercase tracking-wider ml-1">Password</label>
                    <div className="relative">
                      <Lock size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-blue-400/80" />
                      <input
                        type="password"
                        required
                        className="w-full pl-9 pr-3.5 py-3.5 bg-slate-950/80 border border-white/10 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 text-white focus:outline-none transition-all font-bold text-xs rounded-2xl shadow-inner"
                        placeholder="••••••••"
                        value={formData.password}
                        onChange={(e) => setFormData({...formData, password: e.target.value})}
                      />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Key size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-blue-400/80" />
                      <input
                        type="password"
                        required
                        inputMode="numeric"
                        pattern="\d{4}"
                        className="w-full pl-9 pr-3 py-3.5 bg-slate-950/80 border border-white/10 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/10 text-white focus:outline-none transition-all font-bold text-xs text-center tracking-[0.4em] rounded-2xl shadow-inner"
                        placeholder="0000"
